// app/page.tsx (Corregido y Sincronizado)
...
// Iconos necesarios para el resumen inferior
...
const IconFuelAlert = () => (<svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-red-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg>);
...
export default function Home() {
  ...
  const handleResetTrip = () => {
      if (confirm("¿Borrar viaje y empezar de cero?")) { localStorage.removeItem('caracola_trip_v1'); window.location.reload(); }
  };

  const handleLoadCloudTrip = (tripData: any, tripId: number) => {
      if (tripData) {
          setFormData(tripData.formData); setResults(tripData.results); setCurrentTripId(tripId); 
  ...
  const handleShareTrip = async () => {
    if (!currentTripId) return alert("Guarda primero.");
    ...
    navigator.clipboard.writeText(shareUrl).then(() => alert(`🔗 Copiado:\n${shareUrl}`));
  };
  ...
  const handleSaveToCloud = async () => {
    ...
    try {
        if (currentTripId) {
            if (confirm(`¿Sobrescribir viaje (ID: ${currentTripId})?`)) {
                await supabase.from('trips').update({ name: tripName, trip_data: tripPayload, updated_at: new Date().toISOString() }).eq('id', currentTripId);
                alert("✅ Actualizado.");
            } else {
                const { data } = await supabase.from('trips').insert([{ name: tripName + " (Copia)", trip_data: tripPayload, user_id: session.user.id }]).select();
                if (data) setCurrentTripId(data[0].id);
                alert("✅ Copia guardada.");
            }
        } else {
            const { data } = await supabase.from('trips').insert([{ name: tripName, trip_data: tripPayload, user_id: session.user.id }]).select();
            if (data) setCurrentTripId(data[0].id);
            alert("✅ Guardado.");
        }
  ...
  const calculateRoute = async (e: React.FormEvent) => {
    ...
    try {
      ...
      for (let i = 0; i < route.legs.length; i++) {
        ...
        const getCityAndProvinceLoop = async (lat: number, lng: number): Promise<string> => {
            const geocoder = new google.maps.Geocoder();
            try {
              const response = await geocoder.geocode({ location: { lat, lng } });
              if (response.results[0]) {
                const comps = response.results[0].address_components;
                return comps.find(c => c.types.includes("locality"))?.long_name || comps.find(c => c.types.includes("administrative_area-level-2"))?.long_name || "Punto Ruta";
              }
            } catch (e) { }
            return "Parada Ruta";
        };
        ...
        for (let j = 0; j < legPoints.length - 1; j++) {
            const point1 = legPoints[j]; const point2 = legPoints[j+1];
            ...
            if (legAccumulator + segmentDist > maxMeters) {
                ...
                const locationString = await getCityAndProvinceLoop(lat, lng);
  ...
  if (!isLoaded) return <div className="flex justify-center items-center h-screen bg-gray-50 text-red-600 font-bold text-xl animate-pulse">Cargando CaraCola...</div>;
    ...
    <main className="min-h-screen bg-gray-50 flex flex-col items-center py-8 px-4 font-sans text-gray-900">
      ...
      <div className="w-full max-w-6xl space-y-6">
        ...
        <TripForm 
            ...
            loading={loading} onSubmit={calculateRoute} 
            showWaypoints={showWaypoints} setShowWaypoints={setShowWaypoints} 
        />
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div className="lg:col-span-2 h-[500px] bg-gray-200 rounded-xl shadow-lg overflow-hidden border-4 border-white relative no-print">
                        <GoogleMap mapContainerStyle={containerStyle} center={center} zoom={6} onLoad={map => { setMap(map); if (mapBounds) map.fitBounds(mapBounds); }}>
                            ...
                            {results.dailyItinerary?.map((day, i) => day.coordinates && <Marker key={`itinerary-${i}`} position={day.coordinates} icon={day.type === 'tactical' ? ICONS_ITINERARY.tactical : ICONS_ITINERARY.startEnd} title={day.to} />)}
                            ...
                            {Object.keys(places).map((key) => {
                                ...
                                if (toggles[type] || type === 'camping') {
                                    if (savedOfType.length > 0 && type !== 'tourism' && type !== 'custom') listToRender = savedOfType;
                                    else if (type === 'custom') listToRender = savedOfType;
                                    else listToRender = [...savedOfType, ...places[type]]; 
                                } else { listToRender = savedOfType; }
                                ...
                                <InfoWindow position={hoveredPlace.geometry.location} onCloseClick={() => setHoveredPlace(null)} options={{ disableAutoPan: true, pixelOffset: new google.maps.Size(0, -35) }}>
                                    <div className="p-0 w-[200px] overflow-hidden">
                                        {hoveredPlace.photoUrl ? <img src={hoveredPlace.photoUrl} alt={hoveredPlace.name} className="w-full h-24 object-cover rounded-t-lg" /> : 
                                            <div className="w-full h-24 bg-gray-100 flex items-center justify-center text-4xl">{hoveredPlace.type === 'custom' ? '⭐' : hoveredPlace.type === 'camping' ? '🚐' : hoveredPlace.type === 'restaurant' ? '🍳' : '📍'}</div>}
                    ...
                    <div className="lg:col-span-1 bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden flex flex-col h-[500px] print:h-auto print:overflow-visible">
                        <div className='p-0 h-full overflow-hidden print:h-auto print:overflow-visible'>
                            {selectedDayIndex === null ? (
                                ...
                                <DaySpotsList day={results.dailyItinerary![selectedDayIndex]} places={places} loading={loadingPlaces} toggles={toggles} auditMode={auditMode} onToggle={handleToggle} onAddPlace={handleAddPlace} onRemovePlace={handleRemovePlace} onHover={setHoveredPlace} />
                            )}
                        </div>
                    </div>
                </div>
