...
export default function ServerStartup({ children }: ServerStartupProps) {
  ...
  const handleStartServer = async () => {
    if (!isLocalhost) {
      setError('Solo funciona en localhost');
      return;
    }
    ...
    try {
      ...
      if (data.running || data.status === 'starting') {
        ...
        setError(data.error || 'Error al arrancar');
        setIsStarting(false);
      }
    } catch (err) {
      setError('Error');
  ...
  return (
    <>
      {/* Floating Server Button - SMALLER */}
      {showButton && typeof window !== 'undefined' && (
        <div className="fixed bottom-3 right-3 z-50">
          <button
            onClick={handleStartServer}
            disabled={isStarting || !isLocalhost}
            title={
              isStarting
                ? 'Iniciando...'
                : isLocalhost
            ...
            className={`
              ...
              ${
                isStarting
                  ? 'bg-yellow-500 text-white w-7 h-7 cursor-wait'
                  : !isLocalhost
                    ? 'bg-gray-400 text-white w-6 h-6 cursor-not-allowed opacity-50'
                    : isServerReady
                      ? 'bg-green-500 text-white w-6 h-6 cursor-default opacity-60'
                      : 'bg-red-500 hover:bg-red-600 text-white w-7 h-7 cursor-pointer'
              }
            `}
          >
            {isStarting ? (
          ...
          {/* Error Toast - SMALLER */}
          {error && (
            <div className="mt-2 bg-red-50 border border-red-200 rounded p-2 max-w-xs shadow-lg">
