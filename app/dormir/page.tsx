...
// Estilos para el contenedor del mapa (Altura forzada para evitar errores)
...
// --- 4. HOOK: CARGA GOOGLE MAPS NATIVAMENTE (Sin librerías externas) ---
...
const Park4NightCard = ({ query }: { query: string }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Usamos la API de Custom Search
  ...
  if (loading) return <div className="animate-pulse p-4 bg-gray-100 rounded-lg h-24 w-full mt-3 flex items-center justify-center text-gray-400 text-sm">Buscando mejor opción...</div>;
  ...
  if (!data) {
    ...
    return (
      <a href={fallbackLink} target="_blank" rel="noopener noreferrer" className="block p-4 border-2 border-dashed border-gray-300 rounded-lg text-center hover:bg-gray-50 mt-3">
        <span className="text-gray-500">🔍 Buscar en Google (Park4Night)</span>
      </a>
    );
  }

  // Intentamos sacar la imagen del resultado
      ...
      <div className="flex h-32">
        {image && (
          <div className="w-1/3 bg-gray-200 relative overflow-hidden">
             <img src={image} alt={data.title} className="object-cover w-full h-full group-hover:scale-105 transition-transform duration-500" />
          </div>
        )}
        <div className={`p-3 flex flex-col justify-between ${image ? 'w-2/3' : 'w-full'}`}>
          ...
          <div className="flex items-center text-xs font-bold text-orange-500 mt-2">
            <span className="bg-orange-100 px-2 py-1 rounded-full">Ver Ficha ➜</span>
          </div>
        </div>
      </div>
    </a>
    ...
    <div className={`p-6 rounded-xl h-full border-l-4 shadow-sm transition-all ${day.isDriving ? 'bg-blue-50 border-blue-600' : 'bg-orange-50 border-orange-600'}`}>
      ...
      {day.isDriving && (
        <p className="text-3xl font-extrabold text-green-700 mb-6">
          {day.distance.toFixed(0)} <span className="text-sm font-normal text-gray-500">km hoy</span>
        </p>
      )}
...
export default function Home() {
  const isLoaded = useGoogleMapsScript(process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY || '');
  ...
  useEffect(() => {
    if (isLoaded && mapRef.current && !mapInstance) {
      const map = new (window as any).google.maps.Map(mapRef.current, {
        center, zoom: 6, mapTypeControl: false, streetViewControl: false, fullscreenControl: false,
      });
      const renderer = new (window as any).google.maps.DirectionsRenderer({
        map, suppressMarkers: false, polylineOptions: { strokeColor: "#2563EB", strokeWeight: 5 }
      });
      setMapInstance(map);
  ...
  const calculateRoute = async (e: React.FormEvent) => {
    ...
    try {
      ...
      for (let i=0; i<route.legs.length; i++) {
        ...
        for (let j=0; j<pts.length-1; j++) {
          const d = (window as any).google.maps.geometry.spherical.computeDistanceBetween(pts[j], pts[j+1]);
          if (legAcc + d > maxM) {
    ...
    } catch (e) { setResults(p => ({...p, error: "No se pudo calcular la ruta."})); }
